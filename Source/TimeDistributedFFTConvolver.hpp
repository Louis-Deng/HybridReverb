 ...
 , mCurrentInputIndex(0)
{
    ...
    mOutputReal.reset( new juce::AudioBuffer<FLOAT_TYPE>(1, 2 * partitionSize) );
    checkNull(mOutputReal.get());
    mOutputReal->clear();
    mOutputImag.reset( new juce::AudioBuffer<FLOAT_TYPE>(1, 2 * partitionSize) );
    checkNull(mOutputImag.get());
    mOutputImag->clear();
    ...
    mPreviousTail.reset( new juce::AudioBuffer<FLOAT_TYPE>(1, partitionSize) );
    checkNull(mPreviousTail.get());
    mPreviousTail->clear();
}
...
{
    juce::ReferenceCountedObjectPtr<RefCountedAudioBuffer<FLOAT_TYPE> > temp;
    int partitionSize = 4 * mNumSamplesBaseTimePeriod;
...
{
    juce::ReferenceCountedObjectPtr<RefCountedAudioBuffer<FLOAT_TYPE> > temp = mBuffersReal[0];
