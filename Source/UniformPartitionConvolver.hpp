...
: mCurrentInputSegment(0)
{
    ...
    mOutputReal.reset( new juce::AudioBuffer<FLOAT_TYPE>(1, 2 * mBufferSize) );
    checkNull(mOutputReal.get());
    
    mOutputImag.reset( new juce::AudioBuffer<FLOAT_TYPE>(1, 2 * mBufferSize) );
    checkNull(mOutputImag.get());
    mOutputImag->clear();
    
    mPreviousOutputTail.reset( new juce::AudioBuffer<FLOAT_TYPE>(1, mBufferSize) );
    checkNull(mPreviousOutputTail.get());
